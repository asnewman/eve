...
async function addToExistingProject(
  ...
  const addOptions: Parameters<typeof addAgentToProject>[0] = {
    projectRoot: targetPath,
    model: DEFAULT_AGENT_MODEL_ID,
    packageManager: manager.kind,
  };
  if (evePackage !== undefined) {
    addOptions.evePackage = evePackage;
  }
  const result = await dependencies.addAgentToProject(addOptions);
  return {
    packageManager: manager.kind,
...
async function scaffoldProject(
  ...
  try {
    ...
    const scaffoldOptions: Parameters<typeof scaffoldBaseProject>[0] = {
    ...
    if (evePackage !== undefined) {
      scaffoldOptions.evePackage = evePackage;
    }
