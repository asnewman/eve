...
## External pull requests and secret-backed CI

Some CI jobs require credentials for model providers or Vercel preview
deployments. Maintainers do not run secret-backed evals or deploys against
untrusted fork code until the security impact has been reviewed.

External fork pull requests may run a reduced CI set first. If a change needs
secret-backed evals or deploy verification, a maintainer should use a trusted
branch or another maintainer-controlled process after reviewing the code.
